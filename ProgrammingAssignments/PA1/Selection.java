public class Selection extends Partition {
	...
	private int select(int left, int right, int k) {
		...
		int pivot = generateRandomPivot(left, right);
		int partitionIndex = partition(left, right, pivot);
		if(k == partitionIndex - left + 1) return pivot;
		else if(k < partitionIndex - left + 1) return select(left, partitionIndex - 1, k);
		else return select(partitionIndex + 1, right, k - (partitionIndex - left + 1));
	}
}